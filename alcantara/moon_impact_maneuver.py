...
#filename = '../kos/jacy1/orbit.txt'
filename = '../kos/orbit.txt'
theta_rel = 135
delta_v_target = 3.430
delta_t_min = 0.5 *24*3600
delta_t_guess = 1.0 *24*3600
delta_t_max = 1.5 *24*3600
...
def extract_values(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            # Skip empty lines and lines with only hyphens
            ...
            # Remove units (km, °, s) and convert to float
            value = value.split()[0]
            try:
                value = float(value)
                values.append(value)
...
rp = earth.radius_from_altitude(values[0])
ra = earth.radius_from_altitude(values[1])
i = values[4]
Omega0 = values[5]
...
rp = earth.radius_from_altitude(values[9])
ra = earth.radius_from_altitude(values[10])
i = values[13]
Omega0 = values[14]
omega0 = values[15]
theta0 = values[16]
...
print("###############################################")
print("t_clock_burn: ", f"{int(t_clock_burn//3600)}h {int((t_clock_burn%3600)//60)}m {t_clock_burn%60:.2f}s")
print("TNB:", np.round(1000 * parking_orbit.convert_RTN_to_TNB(burn.RTN, t_clock=t_clock_burn), 3))
print("###############################################")
