...
set stop_burn_tol to 0. // tolerance for the burn (delta_v before reaching target to stop the burn)
...
until false{
    ...
    set m0 to mf. // update the initial mass
    print_burn_time().
    print_alignment().
...
set active_engines_isp to 0.
for engine in active_engines {
    set active_engines_isp to active_engines_isp + engine:visp.
}
set active_engines_isp to active_engines_isp / active_engines:length. // ISP médio

set burn_dv to rcs_dv.
set m0 to ship:mass.

until burn_dv >= target_dv - stop_burn_tol {
    local m1 to ship:mass.
    local t1 to time:seconds.
    local dt to t1-t0.
    
    
    // Opção 1: Usando massa média para melhor aproximação
    //local m_avg to (m1-m0)/2 + m0.
    //local thrust_avg to ship:thrust.
    //local dv_increment to (thrust_avg / m_avg) * dt.
    



    // Opção 2: Correção de segunda ordem usando Runge-Kutta 2
    local k1 to (ship:thrust / m0).
    local m_mid to (m1 - m0) / 2 + m0.
    local k2 to (ship:thrust / m_mid).
    local dv_increment to (k1 + k2) * dt / 2.

    set burn_dv to burn_dv + dv_increment.
    set m0 to m1.
    set t0 to t1.
