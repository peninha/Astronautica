...
local delta_v_vec is ship:prograde.
local node_eta is 5380.
...
function print_alignment {
    local align to vang(delta_v_vec, ship:facing:vector).
...
lock t_burn to (node_eta - half_dv_time).
...
local target_dv to delta_v_vec:mag.
...
until burn_dv >= target_dv - stop_burn_tol {
    ...
    print "           deltav:mag: " + delta_v_vec:mag at (0,14).
