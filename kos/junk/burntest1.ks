...
until burn_dv >= target_dv - stop_burn_tol {
    ...
    //local dt to t1-t0.
    //local dv to (ship:thrust/ship:mass)*dt.
    //set burn_dv to burn_dv + dv. 
    //set t0 to t1.

    local m1 to ship:mass.
    set ve to active_engines_isp * g0.
    set burn_dv to burn_dv + ve * ln(m0/m1).
    set m0 to m1.
    ...
    print "           Engine ISP: " + active_engines_isp at (0,15).
}
