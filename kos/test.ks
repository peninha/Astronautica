...
local setup_time to 75. // setup time for the burn
local rcs_spin_time to 40. // time to spin the ship before the burn
local ullage_time to 7. // ullage time for the burn
//local rcs_isp to 136.5. // ISP for the RCS burn
local g0 to constant():G0. // gravity constant
local stop_burn_tol to 1.7. // tolerance for the burn (delta_v before reaching target to stop the burn)
//local total_dv to 3224. // total delta-v for the burn
//local stage_mass to 0.420. // mass of the spin burn stage
local stage_mass to 0.519. // mass of the spin burn stage
local burn_delay to 0. //2.2. // delay for the burn
local compensate_dv to 0. // compensate for the burn

local target_dv to nd:deltav:mag + compensate_dv.

list RCS in rcs_list.
local rcs_thrust to 4 * rcs_list[0]:availablethrust.
...
active_engines:add(engine_list[1]).
...
if rcs_spin_time > 0 {
    ...
    print_status("Waiting for burn time...             ").
}
